#!/bin/python3
	...
	class Seabot2Waypoint(Seabot2Data):
	    def __init__(self, bag_path="", topic_name=""):
	    	...
	    	self.mission_enable = np.empty([self.nb_elements], dtype=boolean)
	    	self.enable_thrusters = np.empty([self.nb_elements], dtype=boolean)
	    	...
	    	self.seafloor_landing = np.empty([self.nb_elements], dtype=boolean)
