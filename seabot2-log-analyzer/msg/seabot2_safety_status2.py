...
class Seabot2SafetyStatus2(Seabot2Data):
    def __init__(self, bag_path="", topic_name="", start_date=datetime.datetime(2019, 1, 1)):
        ...
        self.flash_enable = np.empty([self.nb_elements], dtype='bool')
        self.chirp_enable = np.empty([self.nb_elements], dtype='bool')
        self.cpu = np.empty([self.nb_elements], dtype='float')
        self.ram = np.empty([self.nb_elements], dtype='float')
        self.hdd = np.empty([self.nb_elements], dtype='float')
    ...
    def process_message(self, msg):
        ...
        self.flash_enable[self.k] = msg.flash_enable
        self.chirp_enable[self.k] = msg.chirp_enable
        self.cpu[self.k] = msg.cpu
        self.ram[self.k] = msg.ram
        self.hdd[self.k] = msg.hdd
    ...
    def resize_data_array(self):
        ...
        self.flash_enable = np.resize(self.flash_enable, self.k)
        self.chirp_enable = np.resize(self.chirp_enable, self.k)
        self.cpu = np.resize(self.cpu, self.k)
        self.ram = np.resize(self.ram, self.k)
        self.hdd = np.resize(self.hdd, self.k)
    ...
    def save_data(self):
        ...
        if not os.path.exists(self.topic_full_dir):
            np.savez_compressed(self.topic_full_dir,
                                ...
                                flash_enable=self.flash_enable,
                                chirp_enable=self.chirp_enable,
                                cpu=self.cpu,
                                ram=self.ram,
                                hdd=self.hdd,
                                bathy=self.bathy,
    ...
    def load_message_from_file(self):
        ...
        self.flash_enable = data['flash_enable']
        self.chirp_enable = data['chirp_enable']
        self.cpu = data['cpu']
        self.ram = data['ram']
        self.hdd = data['hdd']
