...
class DockAnalysis(Seabot2Dock):
    ...
    def add_temperature_depth(self):
        ...
        if (not data_temp.is_empty()):
            ...
            self.plot_td = pg_temperature_depth.plot(self.temperature, data_kalman.depth[:-1], pen=pen,
                                                     name="Temperature", stepMode=True)
            ...
            pg_depth.plot(data_mission.time, data_mission.depth[:-1], pen=(0, 255, 0), name="depth (target)",
                          stepMode=True)
            dock_temperature_depth.addWidget(pg_depth)

            self.lr_time = pg.LinearRegionItem([0, data_kalman.time[-1]], bounds=[0, data_kalman.time[-1]],
                                               movable=True)
    ...
    def export_temperature(self):
        ...
        # Solution Two
        slice_size = 0.01
        xnew = np.arange(0.5, np.max(self.td_depth), slice_size)
        ynew = np.ones_like(xnew)*20.0
        for idx_start in range(1, len(xnew)):
            idx_in_range = np.where((self.td_depth >= xnew[idx_start - 1]) & (self.td_depth < xnew[idx_start]))
            val_mean = np.mean(self.td_temperature[idx_in_range])
            if np.isfinite(val_mean):
                ynew[idx_start - 1] = val_mean
            else:
                if idx_start > 1:
                    ynew[idx_start - 1] = ynew[idx_start-2]
        ...
        filtered_signal, _ = lfilter(b, a, ynew, zi=zi*ynew[0])

        import matplotlib.pyplot as plt
        plt.plot(self.td_depth, self.td_temperature, 'o')
        plt.plot(xnew, ynew, '-')
        plt.plot(xnew, filtered_signal, '-')
        plt.show()

        filename = QtGui.QFileDialog.getSaveFileName(self, 'Export temperature', os.path.expanduser("~"),
                                                     filter="*.profile")
        if filename[0]:
        ...
        if (not data_temp.is_empty() and not data_mission.is_empty() and not data_depth.is_empty()):
            ...
            for i in range(np.size(id_new_wp) - 1):
                if not np.isnan(mission_depth_interp[id_new_wp[i]]):
                    pg_temperature_profile.plot(temp_interp[id_new_wp[i]:id_new_wp[i + 1]],
                                                data_depth.depth[id_new_wp[i]:id_new_wp[i + 1]][:-1],
                                                pen=(255 * i / np.size(id_new_wp), 0, 150),
                                                name="Profile " + str(mission_depth_interp[id_new_wp[i]]),
                                                stepMode=True)
    ...
    def add_temperature_keeping(self):
        ...
        if not data_mission.is_empty() and not data_depth.is_empty():
            ...
            pg_temperature.plot(data_temp.time, data_temp.temperature[:-1], pen=(0, 255, 0), name="Temperature",
                                stepMode=True)
    ...
    def add_piston_depth(self):
        ...
        if (not data_kalman.is_empty()):
            ...
            mask_vel = np.where(np.logical_and(
                np.logical_and(np.logical_and(velocity >= -velocity_limit, velocity <= velocity_limit),
                               np.logical_and(depth_error_i >= -depth_error_limit, depth_error_i <= depth_error_limit)),
                depth >= depth_min))

            if (len(mask_vel) > 1):
                pg_compressibility = pg.PlotWidget()
                self.set_plot_options(pg_compressibility)
                pg_compressibility.plot(data_kalman.depth[mask_vel], piston_volume_i[mask_vel], pen=None, symbol='x',
                                        symbol_brush=0.01, name="Piston volume")  # symbolSize = 14
    ...
    def add_replay_kalman_depth(self):
        ...
        if (not data.is_empty()):
            ...
            self.replay_depth = pg_depth.plot(self.sk.msg_time[:-1], self.sk.msg_depth[:-2], pen=(0, 0, 255),
                                              name="depth [simu]", stepMode=True)
            dock_kalman_state.addWidget(pg_depth)

            pg_velocity = pg.PlotWidget()
            self.set_plot_options(pg_velocity)
            pg_velocity.plot(data_fusion.time, data_fusion.velocity[:-1], pen=(0, 255, 0), name="velocity [Filter]",
                             stepMode=True)
            pg_velocity.plot(data.time, data.velocity[:-1], pen=(255, 0, 0), name="velocity [Kalman]", stepMode=True)
            self.replay_velocity = pg_velocity.plot(self.sk.msg_time[:-1], self.sk.msg_velocity[:-2], pen=(0, 0, 255),
                                                    name="velocity [simu]", stepMode=True)
            ...
            self.replay_offset = pg_offset.plot(self.sk.msg_time[:-1], self.sk.msg_offset[:-2] * 1e6, pen=(0, 0, 255),
                                                name="offset [simu]", stepMode=True)
    ...
    def add_replay_kalman_coefficient(self):
        ...
        if (not data.is_empty()):
            ...
            self.replay_coeff_cz = pg_cz.plot(self.sk.msg_time[:-1], self.sk.msg_cz[:-2], pen=(0, 0, 255),
                                              name="cz [simu]", stepMode=True)
            ...
            self.replay_coeff_chi = pg_chi.plot(self.sk.msg_time[:-1], self.sk.msg_chi[:-2], pen=(0, 0, 255),
                                                name="chi [simu]", stepMode=True)
            ...
            self.replay_coeff_chi2 = pg_chi2.plot(self.sk.msg_time[:-1], self.sk.msg_chi2[:-2], pen=(0, 0, 255),
                                                  name="chi2 [simu]", stepMode=True)
    ...
    def add_piston(self):
        ...
        if (not data.is_empty()):
            ...
            pg_depth.plot(data_kalman.time, data_kalman.depth[:-1], pen=(0, 0, 255), name="depth [kalman]",
                          stepMode=True)
            ...
            pg_control_set_point.plot(data_piston.time, -data_piston.position[:-1] * self.tick_to_gram, pen=(255, 0, 0),
                                      name="position (in g)", stepMode=True)
            ...
            pg_control_set_point.plot(data_piston.time,
                                      -data_piston.position[:-1] * self.tick_to_gram + delta_volume[:-1],
                                      pen=(0, 255, 0), name="position density correction (+)", stepMode=True)
            pg_control_set_point.plot(data_piston.time,
                                      -data_piston.position[:-1] * self.tick_to_gram - delta_volume[:-1],
                                      pen=(0, 0, 255), name="position density correction (-)", stepMode=True)
    ...
    def add_replay_kalman_offset(self):
        ...
        if (not data.is_empty()):
            ...
            pg_volume_air.plot(data.time, self.conv_V_air(data.volume_air)[:-1], pen=(0, 255, 0),
                               name="volume_air (eq 1bar, 15°) [real]", stepMode=True)
            self.replay_volume_air = pg_volume_air.plot(self.sk.msg_time[:-1],
                                                        self.conv_V_air(self.sk.msg_volume_air)[:-2], pen=(0, 0, 255),
                                                        name="volume_air (eq 1bar, 15°) [simu]", stepMode=True)
            ...
            pg_offset_total.plot(data.time, data.offset_total[:-1] * 1e6, pen=(0, 255, 0), name="offset total [real]",
                                 stepMode=True)
            self.replay_offset_total = pg_offset_total.plot(self.sk.msg_time[:-1], self.sk.msg_offset_total[:-2] * 1e6,
                                                            pen=(0, 0, 255), name="offset total [simu]", stepMode=True)
    ...
    def open_yaml(self):
        ...
        fileName = QtGui.QFileDialog.getOpenFileNames(self, caption='Param files', directory=os.path.expanduser(
            '~/seabot2/seabot2-ros/src/seabot2/config/'), filter="*.yaml")
        print(fileName)
