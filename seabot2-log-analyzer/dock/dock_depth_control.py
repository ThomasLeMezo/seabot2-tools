...
class DockDepthControl(Seabot2Dock):
    ...
    def plot_regulation_state(self, data_control):
        ...
        for i in np.where(tab[:-1] != tab[1:])[0]:
            self.text_write_plot(pg_regulation_state, data_control.time[i + 1], tab[i + 1],
                                 self.regulation_state[tab[i + 1]])
    ...
    def add_control(self):
        ...
        if (not data.is_empty()):
            ...
            pg_control_set_point.plot(data_piston.time, -data_piston.position_set_point[:-1] * self.tick_to_gram,
                                      pen=(0, 0, 255), name="set point (pic, every 100ms), (in g)", stepMode=True)
            pg_control_set_point.setLabel('left', "Piston state position and set point", "g")
            pg_control_set_point.showGrid(y=True)

            pg_control_set_point.plot(data_control.time, -data_control.piston_set_point[:-1] * self.tick_to_gram,
                                      pen=(0, 255, 0), name="set_point (control) (in g)", stepMode=True)
    ...
    def add_piston_set_point(self):
        ...
        if (not data.is_empty()):
            ...
            pg_control_set_point.plot(data_piston.time,
                                      (data_piston.position_set_point[:-1] - data_piston.position[:-1]),
                                      pen=(0, 255, 0), name="Error of position", stepMode=True)
            ...
            pg_control_piston.plot(data_piston.time, data_piston.motor_speed_set_point[:-1] - 2000.0, pen=(0, 255, 0),
                                   name="motor set point", stepMode=True)
            pg_control_piston.plot(data_piston.time, data_piston.motor_speed[:-1] - 2000.0, pen=(255, 0, 0),
                                   name="motor speed", stepMode=True)
    ...
    def add_alpha(self):
        ...
        if (not data.is_empty() and not data_mission.is_empty()):
            ...
            pg_alpha.plot(data_alpha.time, data_alpha.approach_velocity[:-1], pen=(255, 0, 0), name="approach_velocity",
                          stepMode=True)
            dock_alpha.addWidget(pg_alpha)
            pg_alpha.setXLink(pg_depth)

    def add_alpha2(self):
        dock_alpha2 = Dock("Alpha2")
        ...
        if (not data.is_empty() and not data_mission.is_empty()):
            ...
            pg_depth.plot(data_mission.time, data_mission.depth[:-1], pen=(0, 0, 255), name="depth [target]",
                          stepMode=True)
            dock_alpha2.addWidget(pg_depth)
            ...
            pg_velocity.plot(data_mission.time, -np.array(data_mission.limit_velocity[:-1]), pen=(0, 255, 0),
                             name="target_velocity_min", stepMode=True)
            dock_alpha2.addWidget(pg_velocity)
            pg_velocity.setXLink(pg_depth)
            ...
            self.set_plot_options(pg_control_u)
            pg_control_u.plot(data_control.time, data_control.u[:-1], pen=(0, 255, 0), name="u", stepMode=True)
            dock_alpha2.addWidget(pg_control_u)
            pg_control_u.setXLink(pg_depth)
